...
window.addEventListener('message', function(event) {
    if (typeof event.data === 'string' && event.data.startsWith('embedChannel')) { 
        console.log("*** POST MESSAGE EVENT embedChannel ********************");        
...
$(document).ready(function () {
    ...
    $.get(CDNROOTDIR + 'consoleChannelUI.html', function (contents) {
        ...
        let str = contents.replaceAll("@",CDNROOTDIR).replaceAll("CSSSTYLES",CSSSTYLES);
        HTML = $.parseHTML(str);
        if(hasParent()){
            HTML[3] = "";            
        }
...
// 2
    ...
    console.log("TRACKER SCALE : " + trackerScale);
// 3
...
function validateSite() {
    ...
    console.log("SITE VADIDATION : " + finalUrl);
    $.ajax({
        url: finalUrl
    }).then(function (data) {
        if (data === "HOME") {
...
function configureUI() {
    ...
    console.log("HEIGHT 1 : " + $("#channelTbl").css("height"));
    console.log("WIDTH 1 : " + $("#channelTbl").css("width"));
    console.log("HEIGHT 2 : " + $("#trackingTd").css("height"));
    console.log("WIDTH 2 : " + $("#rootTd").css("width"));
